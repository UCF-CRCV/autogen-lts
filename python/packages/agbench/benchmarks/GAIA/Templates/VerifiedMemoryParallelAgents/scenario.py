...
def _load_decision_transformer():
    ...
    # Prefer a local import if available (e.g., running from the GAIA folder).
    try:
        ...
        gaia_path = "/autogen_python/packages/agbench/benchmarks/GAIA"
...
def _gaia_question_scorer(model_answer: str, ground_truth: str) -> bool:
    ...
    Copied from the official GAIA scorer so that per-run correctness for the
    first-team and aggregated answers can be logged locally in each result
    folder.
    """
...
class SharedMemory:
    ...
    def __init__(
        ...
        self._verify_kept = 0
        self._verify_time_seconds = 0.0
        self._verify_time_calls = 0
        self._inject_time_seconds = 0.0
        self._inject_time_calls = 0
    ...
    async def _verify_step_with_decision_transformer(
        ...
        with torch.no_grad():
            ...
            decision_idx = dt.sample_decision(valid_logits, temperature=1.2)
    ...
    async def store(
        ...
        verify_duration_seconds = time.perf_counter() - verify_start

        async with self._lock:
            self._verify_total += 1
            if keep:
                self._verify_kept += 1
            self._verify_time_seconds += verify_duration_seconds
            self._verify_time_calls += 1
        ...
        self._log_event(
            {
                ...
                "agent_input_source": agent_input_source,
                "kept": keep,
                "verification_duration_seconds": verify_duration_seconds,
            }
        )
        ...
        inject_duration_seconds = None
        async with self._lock:
            inject_start = time.perf_counter()
            self._memory[key] = content
            await self._persist_to_file()
            inject_duration_seconds = time.perf_counter() - inject_start
            self._inject_time_seconds += inject_duration_seconds
            self._inject_time_calls += 1

        if inject_duration_seconds is not None:
            self._log_event(
                {
                    "event": "memory_inject",
                    "timestamp": datetime.utcnow().isoformat(),
                    ...
                    "step_num": step_num,
                    "key": key,
                    "injection_duration_seconds": inject_duration_seconds,
                }
            )
    ...
    if use_decision_transformer:
        ...
        if weights_path:
            ...
            if not os.path.isabs(resolved_path) and not os.path.isfile(resolved_path):
                repo_root = "/autogen_python/packages/agbench/benchmarks/GAIA"
...
""".strip()
    ...
    verification_seconds = getattr(shared_memory, "_verify_time_seconds", 0.0)
    verification_calls = getattr(shared_memory, "_verify_time_calls", 0)
    injection_seconds = getattr(shared_memory, "_inject_time_seconds", 0.0)
    injection_calls = getattr(shared_memory, "_inject_time_calls", 0)
    query_seconds = None
    query_calls = None
    consult_seconds = None
    consult_calls = None
    if events_file_path and os.path.exists(events_file_path):
        total_q = 0.0
        calls_q = 0
        total_consult = 0.0
        calls_consult = 0
        try:
            with open(events_file_path, "r", encoding="utf-8", errors="replace") as f:
                for line in f:
                    try:
                        ev = json.loads(line)
                    except Exception:
                    ...
                    if not isinstance(ev, dict):
                        continue
                    if ev.get("event") == "memory_consult":
                        dur = ev.get("total_seconds")
                        if isinstance(dur, (int, float)):
                            total_consult += float(dur)
                            calls_consult += 1
                    if ev.get("event") != "memory_select":
                        continue
                    dur = ev.get("query_duration_seconds")
                    if isinstance(dur, (int, float)):
                        total_q += float(dur)
                        calls_q += 1
        except Exception:
            ...
            total_consult = 0.0
            calls_consult = 0
        query_seconds = total_q
        query_calls = calls_q
        consult_seconds = total_consult
        consult_calls = calls_consult
    ...
    comparison_metrics: Dict[str, Any] = {
        ...
        "shared_memory": {
            ...
            "verification_fraction_of_total_runtime": (verification_seconds / overall_runtime) if overall_runtime else None,
            "memory_query_calls": query_calls,
            "memory_query_seconds": query_seconds,
            "memory_query_fraction_of_total_runtime": (query_seconds / overall_runtime) if overall_runtime and query_seconds is not None else None,
            "memory_consult_calls": consult_calls,
            "memory_consult_seconds": consult_seconds,
            "memory_consult_fraction_of_total_runtime": (consult_seconds / overall_runtime) if overall_runtime and consult_seconds is not None else None,
            "memory_injection_calls": injection_calls,
            "memory_injection_seconds": injection_seconds,
            "memory_injection_fraction_of_total_runtime": (injection_seconds / overall_runtime) if overall_runtime else None,
        },
    }
    try:
