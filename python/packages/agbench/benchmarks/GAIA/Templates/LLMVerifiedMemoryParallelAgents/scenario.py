...
def _verifier_transcript_line(source: str, content: str, per_line_max: int) -> str:
    text = content if isinstance(content, str) else content_to_str(content)
    if per_line_max > 0 and len(text) > per_line_max:
        text = text[:per_line_max] + "..."
    return f"[{source}]: {text}"


def _truncate_verifier_context_block(text: str, max_chars: int) -> str:
    ...
    if nl != -1 and nl < 240:
        tail = tail[nl + 1 :]
    return f"...[earlier team messages omitted]\n\n{tail}"
...
class SharedMemory:
    ...
    def __init__(
        ...
        verbose: bool = True,
        verifier_conversation_context_max_chars: int = 96_000,
        verifier_transcript_line_max_chars: int = 80_000,
        verifier_step_content_max_chars: int = 12_000,
        verifier_agent_input_max_chars: int = 12_000,
    ):
        ...
        self._verifier_context_max_chars = verifier_conversation_context_max_chars
        self._verifier_transcript_line_max_chars = verifier_transcript_line_max_chars
        self._verifier_step_content_max_chars = verifier_step_content_max_chars
        self._verifier_agent_input_max_chars = verifier_agent_input_max_chars
    ...
    async def _verify_step_with_llm(
        ...
        scap = self._verifier_step_content_max_chars
        icap = self._verifier_agent_input_max_chars
        raw_content = content if isinstance(content, str) else content_to_str(content)
        truncated_content = raw_content[:scap] if scap > 0 else raw_content
        truncated_input = None
        if agent_input is not None:
            raw_in = agent_input if isinstance(agent_input, str) else content_to_str(agent_input)
            truncated_input = raw_in[:icap] if icap > 0 else raw_in
        input_header = ""
        if truncated_input:
        ...
        prior_block = ""
        if (
            self._verifier_context_max_chars > 0
            and conversation_context
            and conversation_context.strip()
        ):
            prior = _truncate_verifier_context_block(
                conversation_context.strip(), self._verifier_context_max_chars
            )
            prior_block = (
                "Below is the same ordered conversation the team's orchestrator has already "
                "processed (read-only copy for this call; it is not written back into the live run).\n\n"
                f"{prior}\n\n"
            )
        if prior_block:
            intro = (
                "You use the same model endpoint as the team orchestrator. Base your judgment on "
                "that conversation copy and the step under review.\n\n"
            )
        else:
            intro = (
                "You use the same model endpoint as the team orchestrator.\n\n"
            )
        messages = [
            UserMessage(
                content=(
...
async def intercept_messages_for_shared_memory(
    ...
    async for message in stream:
        ...
        if source == "user":
            if not _initial_user_message_skipped.get(team_idx, False):
                ...
                transcript_lines.append(
                    _verifier_transcript_line(
                        source, content, shared_memory._verifier_transcript_line_max_chars
                    )
                )
        ...
        if not skip_memory_store:
            try:
                ...
                await shared_memory.store(
                    ...
                    agent_input=agent_input,
                    agent_input_source=agent_input_source,
                    conversation_context=prior if prior.strip() else None,
                )
            except Exception as e:
        ...
        transcript_lines.append(
            _verifier_transcript_line(source, content, shared_memory._verifier_transcript_line_max_chars)
        )
        last_message_source = source
        last_message_content = content
...
if __name__ == "__main__":
    num_teams = 3
    num_answers = 3
    use_decision_transformer = False
